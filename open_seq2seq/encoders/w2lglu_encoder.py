...
class Wave2LetterGLUEncoder(Encoder):
  ...
  def __init__(self, params, model, name="w2l_encoder", mode='train'):
    ...
    * **bn_epsilon** (float) --- epsilon for batch norm. Defaults to 1e-3.
    """
    super(Wave2LetterGLUEncoder, self).__init__(params, model, name, mode)
  ...
  def _encode(self, input_dict):
    ...
    for idx_convnet in range(len(convnet_layers)):
      ...
      for idx_layer in range(layer_repeat):
        in_dim = int(conv_feats.get_shape()[-1])
        out_dim = int(ch_out)

        conv_layer = conv_wn_layer.Conv1DNetworkNormalized(in_dim, out_dim, kernel_size[0], self._mode, "{}{}".format(
                idx_convnet + 1, idx_layer + 1), dropout_keep_prob, padding, False)
        conv_feats = conv_layer(conv_feats)
